...
- Hierarchy integrity (fixed 2026-06-12, «удаление функции не приводит к удалению декомпозиции»): four related bugs in `IDEF0Editor.vue` + `history.js`. (1) `decompose()` derived the child id from the box **index** — after deleting a sibling, indices shift and a new box reused an existing child id, two boxes silently sharing one child diagram; now the id is the first free `A<n>` suffix, and decomposing an already-decomposed box just navigates into its existing child. (2) `_removeSubtree` recursed only via `childMap` (stale on older persisted/imported projects) — now also via the diagrams' own `box.childDiagramId`. (3) `_normalizeHierarchy()` (GC): boxes' `childDiagramId` is the source of truth — drops diagrams unreachable from the root, nulls dangling pointers, rebuilds `childMap`; runs after `deleteSelectedBox`/`onRemoveDecomposition` and on every load/import. (4) Undo was corrupting: `applySnapshot` crashed mid-restore on root diagrams without `boundaryArrows` (defaultProject A0 lacked the key — now guarded + key added), and `resetHistory()` was imported but never called, so Ctrl+Z after navigation spliced **another diagram's** snapshot into the current one — `navigateTo` now resets history on diagram change (snapshots are not tagged with a diagram id). Verified via CDP-driven browser runs (collision, re-decompose, cross-diagram undo, orphan GC after reload); component logic is inline → no node-test coverage
