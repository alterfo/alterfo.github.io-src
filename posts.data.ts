...
function extractExcerpt(content: string, maxLen = 120): string {
  ...
  for (const line of lines) {
    ...
    if (!trimmed || trimmed.startsWith('#') || trimmed.startsWith('`') || trimmed.startsWith('- ') || trimmed.startsWith('* ') || /^\d+\.\s/.test(trimmed) || /^\*+$/.test(trimmed) || trimmed.startsWith('>') || trimmed.startsWith('![') || trimmed.startsWith('<')) continue
    const plain = trimmed
      .replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')
      .replace(/[*_`]/g, '')
      .replace(/&nbsp;/g, ' ')
