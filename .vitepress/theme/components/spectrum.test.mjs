...
test('SPECTRUM has 10 hex colors', () => {
  assert.equal(SPECTRUM.length, 10)
...
test('CANVAS_PALETTE entries mirror SPECTRUM (dual-mirror sync contract)', () => {
  ...
  // With 10 spheres, SPECTRUM and CANVAS_PALETTE are now the same length — fully aligned.
  assert.equal(CANVAS_PALETTE.length, SPECTRUM.length)
...
test('PROJECT_COLORS has all 10 project keys, hex values', () => {
  const keys = ['ar', 'blog', 'idef0', 'journal', 'piano', 'github', 'decisions', 'music', 'finance', 'games']
  for (const k of keys) {
