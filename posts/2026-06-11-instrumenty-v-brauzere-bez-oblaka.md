...
::: details Под капотом
Всё состояние сериализуется и шифруется целиком (AES-GCM, ключ выведен из пароля через PBKDF2, 600 000 итераций), на диск ложится только конверт `{соль, iv, шифртекст}` - ни ключа, ни открытого текста. Проекции в открытый вид нет: названия задач, теги и сроки шифруются наравне с заметками. Перенос между устройствами и бэкап - через экспорт в зашифрованный файл `.planner`, импорт по тому же паролю.
:::
...
## Аудио-реактив, аудиовизуализатор на WebGPU
...
<a href="/ar/" target="_self">→ Аудио-реактив</a> (Chrome 113+, WebGPU) | [Разбор архитектуры](/projects/ar-engine)
